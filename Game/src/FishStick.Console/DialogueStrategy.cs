                    ...
                    {
                        {
                            ...
                            switch (readKeyResult.Key)
                            {
                                case ConsoleKey.UpArrow:
                                    ...
                                    selectedReply =
                                        selectedReply == 0 ? replies.Count - 1 : selectedReply - 1;
                                    ListReplies(currentLine.Replies, selectedReply);
                                    break;
                                case ConsoleKey.DownArrow:
                                    ClearReplies(replies.Count);
                                    selectedReply =
                                        selectedReply == totalLines - 1 ? 0 : selectedReply + 1;
                                    ListReplies(currentLine.Replies, selectedReply);
                                    break;
                                case ConsoleKey.Enter:
                                    ClearReplies(replies.Count);
                                    ConsoleWriter.Write(replies[selectedReply].Text).ToConsole();
                                    Console.WriteLine();
