...
namespace FishStick.Commands
{
  class TakeCommand(PlayerController player, WorldController world) : ICommand
  {
    ...
    void ICommand.Execute(string[] args)
    {
      ...
      IItem? item =
        TakeItemFromScene(targetItemName, currentScene)
        ?? TakeItemFromSceneContainer(targetItemName, currentScene)
        ?? TakeItemFromInventoryContainer(targetItemName);
      if (item == null)
      {
    ...
    {
      IItem? item = currentScene
        .Items
        .Find(item => item.Name == targetItemName && item.Hidden == false);
      if (item != null)
      {
    ...
    private IItem? TakeItemFromSceneContainer(string targetItemName, IScene currentScene)
    {
      ...
      List<IContainer> containers = currentScene
        .Items
        .OfType<IContainer>()
        .ToList()
        .FindAll(container => container.Locked == false);
    ...
    private IItem? TakeItemFromInventoryContainer(string targetItemName)
    {
      ...
      List<IContainer> containers = _player
        .GetInventory()
        .OfType<IContainer>()
        .ToList()
        .FindAll(container => container.Locked == false);
